...
class InvMelSpectrogram(nn.Module):
    ...
    def _mel_to_linear(self, mel_spec):
        ...
        return torch.max(torch.as_tensor(1e-10, dtype=torch.float32), dot)
    ...
    def _db_to_amp(x):
        # type: (Tensor) -> Tensor
        return torch.pow(torch.as_tensor(10.0, dtype=torch.float), x.float() * 0.05)
    ...
    def inference(self, text):
        ...
        # normalize samples
        wav_norm = wav * float(32767. / float(torch.max(torch.as_tensor(0.01, dtype=torch.float32), torch.max(torch.abs(wav)))))
    ...
    def find_endpoint(self, wav, threshold_db=-40, min_silence_sec=0.8):
        ...
        threshold = self._db_to_amp(torch.as_tensor(threshold_db, dtype=torch.float))
        ret = len(wav)
